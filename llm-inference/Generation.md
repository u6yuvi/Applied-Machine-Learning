...
- **Prefill is Compute-Bound:** We process the whole prompt at once. The GPU's math cores are maxed out doing giant matrix multiplications.
- **Decode is Memory-Bound:** The math is tiny (1 × d), but the GPU has to fetch the massive KV Cache from VRAM to its processing cores at every single step. This memory shuffling is the main bottleneck in LLMs today.
...
- **Multi-Query Attention (MQA)** shares 1 KV across all heads.
- **Grouped-Query Attention (GQA)** shares 1 KV across a group of heads (Used in Llama-3, Mistral).

### Handling Fragmentation (PagedAttention)
...
## Next things:
 Continuous Batching (aka In-Flight Batching)
...
Sliding Window Attention (SWA): Used by Mistral. Only keep the last 
W
W
 tokens in the cache (e.g., last 4,096 tokens). Drop the rest.
